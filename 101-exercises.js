...
var isPositiveOdd = function isPostiveOdd(number) {
  return number % 2 > 0;
...
function productOfAll(sequence) {
  if (toString.call(sequence) !== "[object Array]")
    return false;
  var total = 1;
  for(var i = 0; i <sequence.length; i++) {
    if(isNaN(sequence[i])){
      continue;
    } total *= Number(sequence[i]);
  } return total;
}
...
function hasEvens(sequence) {
  return sequence.some(function(value) {
    return value % 2 == 0;
  });
}
...
function getNumberOfBooks(arrays) {
...
function totalOfBookPrices(arrays) {
  var sum = 0;
...
function getAverageBookPrice(arrays) {
  var total = 0;
  for(var i = 0; i < arrays.length; i++) {
